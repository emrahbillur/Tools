...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --no-sync-esp) SYNC_ESP_TO_ROOT=0; shift 1 ;;
    --no-fix-root-profile) FIX_ROOT_PROFILE=0; shift 1 ;;
...
# ---------- Helpers ----------
bytes_to_sectors() { local bytes="$1" ; local sect="$2"; echo $(( (bytes + sect - 1) / sect )); }
floor_align()      { local v="$1" a="$2"; echo $(( (v / a) * a )); }
ceil_align()       { local v="$1" a="$2"; echo $(( ((v + a - 1) / a) * a )); }
